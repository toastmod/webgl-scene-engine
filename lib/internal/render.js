...
class Uniform {
    ...
    set(value) {
        this.value = value;
        // TODO: check if its more optimal to update on set
        // this.update();
    }
...
class Program {
    ...
    constructor(desc, vSrc, fSrc) {
        ...
        this.isReady = Promise.all(sources.map(x => fetch(x).then(r => r.text())))
        .then(([vSrc, fSrc]) => {
            // Init program
            ...
            if (!gl.getProgramParameter(this.program, gl.LINK_STATUS)) {
                console.log('Unable to link the shader program' + gl.getProgramInfoLog(this.program));
                return null;
            }
    
            // Init uniform locations
            for(const uKey in desc.uniforms) {
                this.uniforms[uKey] = new Uniform(this.program, DATA_TYPE[desc.uniforms[uKey]], uKey);
                if(this.uniforms[uKey].location === -1) {
                    console.error("Uniform location for '"+uKey+"' could not be found!");
                }
            }
            ...
            for(let aKey in desc.attributes) {
                ...
                if(this.attributes[aKey] === -1) {
                    console.error("Attribute location for '"+aKey+"' could not be found!");
                }
            }
    ...
    createVAO(attribArrays) {
        ...
        for(let aKey in attribArrays) {
            ...
            if(array == null || array.length <= 0) {
                console.error("Cannot create buffer for '"+aKey+"' from empty object or array!");
                return null;
            }
    ...
    // newUniform(fType, uName, value) {
    //     if(uName in this.uniforms) {
    //         console.error("Tried to create uniform '"+uName+"' but it already exists!");
    //     } else {
    //         this.uniforms[uName] = new Uniform(fType, value, gl.getUniformLocation(program, name));
    //     }
    // }
}
...
class GPUState {
    ...
    createFramebuffer(name, targetTextureWidth, targetTextureHeight) {
        let targetTexture = this.createTexture(targetTextureWidth, targetTextureHeight);
    ...
    //         let program = this.programs[pKey];
    //         gl.useProgram(program.program);
    //         for(let uKey in program.uniforms) {
    //             program.uniforms[uKey].update();
    //         }
    //     }
    // }
    ...
    async init(desc) {
        ...
        for(let mKey in models) {
            ...
            let texture = null;
            if("texture" in model) {
                let textureImage = await loadImage(texDataUrl);

                // Create and write to texture on GPU
                texture = this.createTexture(textureImage.width, textureImage.height);
